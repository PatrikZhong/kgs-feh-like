...
func _act(enemy: Unit, map) -> void:
	...
	if not enemy.has_attacked:
		var dist := _manhattan(enemy.grid_cell, nearest.grid_cell)
		if dist <= enemy.data.attack_range:
			map.perform_combat(enemy, nearest)

func _find_nearest(enemy: Unit, player_units: Array) -> Unit:
	var nearest: Unit = null
	var min_dist := INF
	for pu in player_units:
		if not is_instance_valid(pu):
			continue
		var d := _manhattan(enemy.grid_cell, pu.grid_cell)
		if d < min_dist:
			min_dist = d
			nearest = pu
	return nearest

func _move_toward(enemy: Unit, target: Unit, map) -> void:
	...
	var path := grid_mgr.get_path_to(enemy.grid_cell, target.grid_cell)
	...
	var steps := mini(enemy.data.move_range, path.size() - 1)
